  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/traductores`,
        {
          method: "POST",
    ...
    return (
      <main className="min-h-screen pt-24 pb-24 px-4 sm:px-6 flex items-center justify-center">
        <motion.div
          ...
          className="glass-card rounded-3xl p-10 max-w-lg text-center"
        >
          ...
          <p className="text-slate-300 mb-8">
            Tu perfil ahora está visible en la Red de Traductores. Guarda bien
            tu PIN de seguridad por si necesitas eliminar tu registro.
          </p>
          <Link
            href="/traductores"
            className="px-6 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-medium transition-colors w-full block"
          >
            Ver Directorio
          </Link>
        </motion.div>
    ...
    <main className="min-h-screen pt-24 pb-24 px-4 sm:px-6 lg:px-8 max-w-3xl mx-auto">
      ...
      >
        ...
        <p className="text-slate-400 text-lg">
          Tu habilidad para hablar otros idiomas puede salvar vidas conectando
          equipos internacionales con víctimas locales.
        </p>
      </motion.div>
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <div>
            ...
            <input
              ...
              className="w-full bg-slate-800/50 border border-slate-600 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-blue-500 focus:outline-none"
              placeholder="Ej. Ana Suárez"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-2">
            ...
            <input
              ...
              onChange={handleChange}
              className="w-full bg-slate-800/50 border border-slate-600 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-blue-500 focus:outline-none"
              placeholder="Ej. Inglés, Francés nativo"
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <input
              ...
              onChange={handleChange}
              className="w-full bg-slate-800/50 border border-slate-600 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-blue-500 focus:outline-none"
              placeholder="Ej. 24/7, Solo Tardes"
            />
          </div>
        </div>
        ...
        <div>
          ...
          <input
            ...
            onChange={handleChange}
            className="w-full bg-slate-800/50 border border-slate-600 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-blue-500 focus:outline-none"
            placeholder="Ej. Caracas (UTC-4) o España"
          />
        </div>
        ...
        <div className="p-4 rounded-xl bg-slate-800/80 border border-slate-700">
          ...
          <p className="text-xs text-slate-400 mb-3">
            Ingresa 4 números que usarás más adelante si deseas eliminar tu
